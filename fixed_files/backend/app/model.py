...
ROOT      = Path(__file__).resolve().parents[2]  # backend/app/model.py -> repo root
...
def predict_price(commodity: str, market: str, year: int, month: int) -> dict:
    ...
    return {
        ...
        "currency":     "UGX",
        "price_lag1":   round(row["price_lag1"], 2),
    }
...
def detect_fake(features: dict) -> dict:
    ...
    # IsolationForest is an unsupervised anomaly detector — it has no
    # predict_proba(). predict() returns -1 (anomaly/likely fake) or 1 (normal).
    # score_samples() gives a raw anomaly score (higher = more normal); we
    # min-max-normalize it against the training threshold to get a 0-1
    # "fake probability" that's comparable across calls.
    raw_label   = int(_fake_model.predict(X_scaled)[0])
    is_fake     = raw_label == -1
    anomaly_score = float(_fake_model.decision_function(X_scaled)[0])  # >0 normal, <0 anomaly

    # squash decision_function output (~[-0.5, 0.5] typical range) into (0, 1)
    fake_probability = float(1 / (1 + np.exp(anomaly_score * 10)))
    genuine_probability = 1.0 - fake_probability

    return {
        "is_fake":          is_fake,
        "confidence":       round(max(fake_probability, genuine_probability), 4),
        "fake_probability": round(fake_probability, 4),
        "genuine_probability": round(genuine_probability, 4),
        "risk_level": (
            "HIGH"   if fake_probability > 0.75 else
            "MEDIUM" if fake_probability > 0.40 else
            "LOW"
        ),
    }
